...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h4 class="m-0 text-dark">{{ (isset($profile_professional_activitys) && $profile_professional_activitys->count() > 0)? "Update":"Add" }} Professional Activity Info</h4>
        </div>
        <div class="col-sm-6">
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h5>
          @if((isset($profile_professional_activitys) && $profile_professional_activitys->count() > 0))
          @lang('Update') @lang('Professional Activity Info')
          @else
          @lang('Add') @lang('Professional Activity Info')
          @endif
      ...
      <div class="tab">
        ...
        @if(@$profile_id)
        <a class="tablinks active" style="color: black;">Professional Activity</a>
        @endif
      ...
      <div class="card-body">
        <form method="post" action="{{(@$profile_id)?route('profile_professional_activity_info_update',$profile_id):route('profile_professional_activity_info_store')}}" id="" enctype="multipart/form-data">
            @csrf
            <div id="professional_activity_info" class="tabcontent">
                <h4>Professional Activity Info</h4>
                {{-- class="form-row" removed for conflict of minimizing ckeditor features --}}
                <div class="" id="add_education_extra_div">
                  @if(!empty($profile_professional_activitys))
                    @foreach($profile_professional_activitys as $key => $profile_professional_activity)
                    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    ...
                                    <input type="hidden" value="{{ !empty($profile_professional_activity->id)? $profile_professional_activity->id : "" }}" name="professional_activity_id[{{$key}}]">
                                    <textarea id="" name="ProfessionalActivity[{{$key}}]" class="form-control textarea">{{ !empty($profile_professional_activity->ProfessionalActivity)? $profile_professional_activity->ProfessionalActivity : "" }}</textarea>
                                    @error('ProfessionalActivity')
                        ...
                        <div class="col-1" style="text-align: center;writing-mode: vertical-lr;background: #e6e6b9;border-radius: 0px 10px 10px 0px;">
                            ...
                            <div class="form-group col-md-2">
                                {{-- @if($key != 0) --}}
                                <a id="delete" title="Delete" data-id="{{@$profile_professional_activity->id}}" data-token="{{csrf_token()}}" data-route="{{route('remove_profile_professional_activity')}}"> <i class="btn btn-danger fas fa-trash"> </i>  </a>
                                {{-- @endif --}}
                  ...
                  @if(empty($profile_professional_activitys) || count($profile_professional_activitys) == 0)
                  <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                      <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                          <div class="row">
                              <div class="form-group col-sm-12">
                                  <label>Professional Activity</label>
                                  <textarea id="" name="ProfessionalActivity[5000]" class="form-control textarea">{{ @$profile_professional_activity->ProfessionalActivity }}</textarea>
                                  @error('ProfessionalActivity')
                                  <span class="invalid-feedback" role="alert">
                                      <strong>{{ $message }}</strong>
                                  </span> @enderror
                ...
                <div class="form-row">
                  <div class="form-group col-md-3" style="margin-top: 20px;">
                      <button type="submit" class="btn btn-primary">{{(isset($profile_professional_activitys) && $profile_professional_activitys->count() > 0)?"Update":"Save"}}</button>
                  </div>
                </div>
            </div>
    ...
    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
            <div class="row">
                <div class="form-group col-sm-12">
                    <label>Professional Activity</label>
                    <textarea id="" name="ProfessionalActivity[@{{counter}}]" class="form-control textarea"></textarea>
                    @error('ProfessionalActivity')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span> @enderror
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on("click",".add_education_extra",function(){
            ...
            $('.select2').select2();
            CKEDITOR.replace('ProfessionalActivity['+counter+']');
            counter ++;
        });
