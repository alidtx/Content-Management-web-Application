...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h4 class="m-0 text-dark">{{ (isset($profile_memberships) && $profile_memberships->count() > 0)? "Update":"Add" }} Membership Info</h4>
        </div>
        <div class="col-sm-6">
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h5>
          @if((isset($profile_memberships) && $profile_memberships->count() > 0))
          @lang('Update') @lang('Membership Info')
          @else
          @lang('Add') @lang('Membership Info')
          @endif
      ...
      <div class="tab">
        ...
        @if(@$profile_id)
        <a class="tablinks active" style="color: black;">Membership</a>
        @endif
      ...
      <div class="card-body">
        <form method="post" action="{{(@$profile_id)?route('profile_membership_info_update',$profile_id):route('profile_membership_info_store')}}" id="" enctype="multipart/form-data">
            @csrf
            <div id="membership_info" class="tabcontent">
                ...
                <div class="" id="add_education_extra_div">
                    @if(!empty($profile_memberships))
                    @foreach($profile_memberships as $key => $profile_membership)
                    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    ...
                                    <input type="hidden" value="{{ !empty($profile_membership->id)? $profile_membership->id : "" }}" name="membership_id[{{$key}}]">
                                    <textarea id="" name="Membership[{{$key}}]" class="form-control textarea">{{ !empty($profile_membership->Membership)? $profile_membership->Membership : "" }}</textarea>
                                    @error('Membership')
                        ...
                        <div class="col-1" style="text-align: center;writing-mode: vertical-lr;background: #e6e6b9;border-radius: 0px 10px 10px 0px;">
                            ...
                            <div class="form-group col-md-2">
                                {{-- @if($key != 0) --}}
                                <a id="delete" title="Delete" data-id="{{@$profile_membership->id}}" data-token="{{csrf_token()}}" data-route="{{route('remove_profile_membership')}}"> <i class="btn btn-danger fas fa-trash"> </i>  </a>
                                {{-- @endif --}}
                    ...
                    @if(empty($profile_memberships) || count($profile_memberships) == 0)
                    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    <label>Membership</label>
                                    <textarea id="" name="Membership[5000]" class="form-control textarea">{{ @$profile_membership->Membership }}</textarea>
                                    @error('Membership')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span> @enderror
                ...
                <div class="form-row">
                  <div class="form-group col-md-3" style="margin-top: 20px;">
                      <button type="submit" class="btn btn-primary">{{(isset($profile_memberships) && $profile_memberships->count() > 0)?"Update":"Save"}}</button>
                  </div>
                </div>
            </div>
    ...
    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
            <div class="row">
                <div class="form-group col-sm-12">
                    <label>Membership</label>
                    <textarea id="" name="Membership[@{{counter}}]" class="form-control textarea"></textarea>
                    @error('Membership')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span> @enderror
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on("click",".add_education_extra",function(){
            ...
            $('.select2').select2();
            CKEDITOR.replace('Membership['+counter+']');
            counter ++;
        });
