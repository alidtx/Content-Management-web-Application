...
<div class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h4 class="m-0 text-dark">{{ (isset($profile_course_taughts) && $profile_course_taughts->count() > 0)? "Update":"Add" }} Course Taught Info</h4>
        </div>
        <div class="col-sm-6">
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h5>
          @if((isset($profile_course_taughts) && $profile_course_taughts->count() > 0))
          @lang('Update') @lang('Course Taught Info')
          @else
          @lang('Add') @lang('Course Taught Info')
          @endif
      ...
      <div class="tab">
        ...
        @if(@$profile_id)
        <a class="tablinks active" style="color: black;">Course Taught</a>
        @endif
      ...
      <div class="card-body">
        <form method="post" action="{{(@$profile_id)?route('profile_course_taught_info_update',$profile_id):route('profile_course_taught_info_store')}}" id="" enctype="multipart/form-data">
            @csrf
            <div id="course_taught_info" class="tabcontent">
                <h4>Course Taught Info</h4>
                {{-- class="form-row" removed for conflict of minimizing ckeditor features --}}
                <div class="" id="add_education_extra_div">
                    @if(!empty($profile_course_taughts))
                    @foreach($profile_course_taughts as $key => $profile_course_taught)
                    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    ...
                                    <input type="hidden" value="{{ !empty($profile_course_taught->id)? $profile_course_taught->id : "" }}" name="course_taught_id[{{$key}}]">
                                    <textarea id="" name="CourseTaught[{{$key}}]" class="form-control textarea">{{ !empty($profile_course_taught->CourseTaught)? $profile_course_taught->CourseTaught : "" }}</textarea>
                                    @error('CourseTaught')
                        ...
                        <div class="col-1" style="text-align: center;writing-mode: vertical-lr;background: #e6e6b9;border-radius: 0px 10px 10px 0px;">
                            ...
                            <div class="form-group col-md-2">
                                {{-- @if($key != 0) --}}
                                <a id="delete" title="Delete" data-id="{{@$profile_course_taught->id}}" data-token="{{csrf_token()}}" data-route="{{route('remove_profile_course_taught')}}"> <i class="btn btn-danger fas fa-trash"> </i>  </a>
                                {{-- @endif --}}
                    ...
                    @if(empty($profile_course_taughts) || count($profile_course_taughts) == 0)
                    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
                        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    <label>Course Taught</label>
                                    <textarea id="" name="CourseTaught[5000]" class="form-control textarea">{{ @$profile_course_taught->CourseTaught }}</textarea>
                                    @error('CourseTaught')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span> @enderror
                ...
                <div class="form-row">
                  <div class="form-group col-md-3" style="margin-top: 20px;">
                      <button type="submit" class="btn btn-primary">{{(isset($profile_course_taughts) && $profile_course_taughts->count() > 0)?"Update":"Save"}}</button>
                  </div>
                </div>
            </div>
    ...
    <div class="row remove_education_extra_div" style="margin-top: 2px;margin-left: 0; margin-right:0;">
        <div class="col-11" style="border: 2px solid #e6e6b9;padding: 20px;border-radius: 10px 0px 0px 10px;">
            <div class="row">
                <div class="form-group col-sm-12">
                    <label>Course Taught</label>
                    <textarea id="" name="CourseTaught[@{{counter}}]" class="form-control textarea"></textarea>
                    @error('CourseTaught')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span> @enderror
...
<script type="text/javascript">
    $(document).ready(function(){
        ...
        $(document).on("click",".add_education_extra",function(){
            ...
            $('.select2').select2();
            CKEDITOR.replace('CourseTaught['+counter+']');
            counter ++;
        });
